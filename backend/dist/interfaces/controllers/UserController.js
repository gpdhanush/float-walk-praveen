...
export async function updateProfile(req, res) {
    try {
        ...
        const { name, storeName, storeAddress, phone, officePhone, gstPercent, gstNumber, logoUrl, theme, themeColor, language, } = req.body;
        ...
        if (storeAddress !== undefined)
            updates.storeAddress = storeAddress;
        if (phone !== undefined)
            updates.phone = phone;
        if (officePhone !== undefined)
            updates.officePhone = officePhone;
        ...
        console.log("[UserController] Profile updated successfully, logoUrl length:", user.logoUrl?.length || 0);
        res.json({ success: true, data: user });
    }
    catch (error) {
        res.status(500).json({ success: false, error: error.message });
