...
export class AnalyticsService {
     ...
     */
    validatePageViewRequest(body) {
        if (!body || typeof body !== 'object') {
            throw new AppError(ErrorCodes.BAD_REQUEST, 'Request body must be valid JSON', 400);
        }
        const req = body;
        // Validate page_path
        if (!req.page_path || typeof req.page_path !== 'string') {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'page_path is required and must be a string', 400);
        }
        const normalizedPath = this.normalizePagePath(req.page_path);
        // Validate visitor_id
        if (!req.visitor_id || typeof req.visitor_id !== 'string') {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'visitor_id is required and must be a string', 400);
        }
        if (!validateUUID(req.visitor_id)) {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'visitor_id must be a valid UUID', 400);
        }
        // Validate referrer (optional)
        let normalizedReferrer;
        if (req.referrer) {
            if (typeof req.referrer !== 'string') {
                throw new AppError(ErrorCodes.VALIDATION_ERROR, 'referrer must be a string', 400);
            }
    ...
    normalizePagePath(path) {
        if (!path.startsWith('/')) {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'page_path must start with /', 400);
        }
        // Remove fragment
        ...
        for (const char of normalized) {
            ...
            if (code < 32 || code === 127) {
                throw new AppError(ErrorCodes.VALIDATION_ERROR, 'page_path contains invalid control characters', 400);
            }
        }
        // Check max length
        if (normalized.length > 512) {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'page_path must be 512 characters or less', 400);
        }
        return normalized;
    }
    /**
     ...
     */
    async processPageView(req, visitorIdentifier) {
        // Check content type
        const contentType = req.get('content-type');
        if (!contentType || !contentType.includes('application/json')) {
            throw new AppError(ErrorCodes.BAD_REQUEST, 'Content-Type must be application/json', 400);
        }
        // Check body size limit (reasonable request size)
        const contentLength = parseInt(req.get('content-length') || '0', 10);
        if (contentLength > 1024) {
            throw new AppError(ErrorCodes.BAD_REQUEST, 'Request body too large', 413);
        }
        // Rate limit check
        if (!this.checkRateLimit(visitorIdentifier)) {
            throw new AppError(ErrorCodes.BAD_REQUEST, 'Too many requests. Please try again later.', 429);
        }
        ...
        // Check deduplication
        const isUnique = await this.repository.checkAndRecordDedup(pageView.visitor_id, pageView.page_path, config.analytics.deduplicationMinutes);
        // Record page view in analytics
        try {
            await this.repository.recordPageView(pageView.page_path, analyticsDate, true, // Always increment total
            isUnique // Only increment unique if first time in window
            );
        }
        catch (error) {
            console.error('[AnalyticsService] Error recording page view:', error);
            throw new AppError(ErrorCodes.INTERNAL_ERROR, 'Failed to record page view', 500);
        }
        return {
            success: true,
            data: { recorded: true },
     ...
     */
    async getAdminAnalytics(fromDate, toDate) {
        // Validate dates
        const dateRegex = /^\d{4}-\d{2}-\d{2}$/;
        if (!dateRegex.test(fromDate) || !dateRegex.test(toDate)) {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'Dates must be in YYYY-MM-DD format', 400);
        }
        if (new Date(fromDate) > new Date(toDate)) {
            throw new AppError(ErrorCodes.VALIDATION_ERROR, 'fromDate must be before or equal to toDate', 400);
        }
