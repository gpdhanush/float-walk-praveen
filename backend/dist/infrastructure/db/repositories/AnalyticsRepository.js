...
export class AnalyticsRepository {
     ...
     */
    async recordPageView(pagePath, analyticsDate, incrementTotal = true, incrementUnique = false) {
        ...
        await pool.execute(`INSERT INTO page_daily_analytics 
       (page_path, analytics_date, total_views, unique_views, created_at, updated_at) 
       VALUES (?, ?, ?, ?, NOW(3), NOW(3))
       ON DUPLICATE KEY UPDATE 
       total_views = total_views + ?,
       unique_views = unique_views + ?,
       updated_at = NOW(3)`, [pagePath, analyticsDate, totalIncrement, uniqueIncrement, totalIncrement, uniqueIncrement]);
    }
    /**
     ...
     */
    async checkAndRecordDedup(visitorId, pagePath, deduplicationMinutes) {
        try {
            ...
            await pool.execute(`INSERT INTO analytics_visitor_dedup 
         (visitor_id, page_path, expires_at, created_at) 
         VALUES (?, ?, ?, NOW(3))`, [visitorId, pagePath, expiresAt]);
     ...
     */
    async getPublicCount(scope = 'lifetime') {
        ...
        const rows = await selectRows(`SELECT 
       ...
       WHERE ${whereClause}`, []);
        ...
        const last30Rows = await selectRows(`SELECT SUM(total_views) as total_views FROM page_daily_analytics 
       WHERE analytics_date >= ?`, [thirtyDaysAgoStr]);
        const yearRows = await selectRows(`SELECT SUM(total_views) as total_views FROM page_daily_analytics 
       WHERE analytics_date >= ?`, [yearStartStr]);
        return {
     ...
     */
    async getPageViewsByDateRange(fromDate, toDate) {
        ...
        const totalRows = await selectRows(`SELECT 
        SUM(total_views) as total_views,
        SUM(unique_views) as unique_visitors
       FROM page_daily_analytics
       WHERE analytics_date >= ? AND analytics_date <= ?`, [fromDate, toDate]);
        const totalData = totalRows[0] || { total_views: 0, unique_visitors: 0 };
        // Today's views
        const todayRows = await selectRows(`SELECT SUM(total_views) as total_views FROM page_daily_analytics 
       WHERE analytics_date = ?`, [todayStr]);
        // Last 7 days views
        const last7Rows = await selectRows(`SELECT SUM(total_views) as total_views FROM page_daily_analytics 
       WHERE analytics_date >= ? AND analytics_date <= ?`, [sevenDaysAgoStr, todayStr]);
        // Last 30 days views
        const last30Rows = await selectRows(`SELECT SUM(total_views) as total_views FROM page_daily_analytics 
       WHERE analytics_date >= ? AND analytics_date <= ?`, [thirtyDaysAgoStr, todayStr]);
        // Per-page breakdown
        const pagesRows = await selectRows(`SELECT 
        page_path,
        SUM(total_views) as total_views,
       ...
       GROUP BY page_path
       ORDER BY total_views DESC`, [fromDate, toDate]);
    ...
    async getCachedData(cacheKey) {
        const rows = await selectRows(`SELECT cache_data, expires_at FROM analytics_cache 
    ...
    async setCachedData(cacheKey, data, ttlSeconds) {
        ...
        // Insert or replace
        await pool.execute(`INSERT INTO analytics_cache (cache_key, cache_data, expires_at, created_at)
       VALUES (?, ?, ?, NOW(3))
       ON DUPLICATE KEY UPDATE
       cache_data = VALUES(cache_data),
       expires_at = VALUES(expires_at),
       created_at = NOW(3)`, [cacheKey, jsonData, expiresAt]);
    }
    /**
     * Clean up expired dedup records.
     */
    async cleanupExpiredDedup() {
        const result = await pool.execute(`DELETE FROM analytics_visitor_dedup WHERE expires_at <= NOW()`, []);
    ...
    async cleanupExpiredCache() {
        const result = await pool.execute(`DELETE FROM analytics_cache WHERE expires_at <= NOW()`, []);
        const affectedRows = result[1]?.affectedRows ?? 0;
        return affectedRows;
    }
}
