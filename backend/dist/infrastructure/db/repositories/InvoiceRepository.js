...
export class InvoiceRepository {
    async create(data) {
        ...
        try {
            ...
            await pool.execute(`INSERT INTO invoices (id, code, customer_id, status, type, total_amount, paid_amount, subtotal, gst_percent, gst_amount, notes, created_by)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                data.code,
                data.customerId,
        ...
        const rows = await selectRows(`SELECT i.id, i.code, i.code AS invoiceNumber, i.customer_id AS customerId, 
    ...
    async update(id, data) {
        ...
        const ignoredFields = [
            ...
            "createdBy",
            "grandTotal",
            "advancePaid", // These are aliases for totalAmount and paidAmount
            "balanceDue",
            "items",
            "payments",
            "customerName",
        ...
        for (const [k, v] of Object.entries(data)) {
            if (v === undefined || ignoredFields.includes(k))
                continue;
            const col = map[k] ?? k;
            fields.push(`${col} = ?`);
            values.push(v);
    ...
    async addItem(data) {
        ...
        try {
            console.log("[InvoiceRepository] Attempting to add item to invoice:", data.invoiceId);
            const legacy = data;
            const unitPrice = data.unitPrice ?? legacy.price ?? 0;
            const scanPrice = data.scanPrice ?? legacy.scan ?? 0;
            await pool.execute(`INSERT INTO invoice_items (id, invoice_id, product_name, quantity, unit_price, scan_price, total_price, product_id)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?)`, [
                id,
                data.invoiceId,
                data.productName,
            ...
            console.error("[InvoiceRepository] ADD ITEM FAILED for invoice:", data.invoiceId);
            console.error("[InvoiceRepository] Error details:", error.message);
            console.error("[InvoiceRepository] Error code:", error.code);
            throw error;
        }
    ...
    async getItems(invoiceId) {
        const rows = await selectRows("SELECT id, invoice_id AS invoiceId, product_name AS productName, quantity, unit_price AS price, scan_price AS scan, total_price AS total, product_id AS productId FROM invoice_items WHERE invoice_id = ? ORDER BY id", [invoiceId]);
        return mapRows(rows);
    }
    ...
    async addPayment(data) {
        const id = data.id ?? randomUUID();
        await pool.execute(`INSERT INTO payments (id, invoice_id, amount, method)
       VALUES (?, ?, ?, ?)`, [id, data.invoiceId, data.amount, data.method]);
