...
export const webResourceDefinitions = {
    enquiries: {
...
export class WebAdminRepository {
    ...
    async list(resource, limit, offset) {
        ...
        const columns = ['id', ...definition.columns, 'created_at', 'updated_at']
            ...
            .join(', ');
        const countRows = await selectRows(`SELECT COUNT(*) AS total FROM ${definition.table}`);
        const rows = await selectRows(`SELECT ${columns} FROM ${definition.table} ORDER BY ${definition.orderBy} LIMIT ? OFFSET ?`, [limit, offset]);
    ...
    async getById(resource, id) {
        ...
        const columns = ['id', ...definition.columns, 'created_at', 'updated_at']
            .map((column) => ['preferred_date', 'review_date'].includes(column) ? `DATE_FORMAT(${column}, '%Y-%m-%d') AS ${column}` : column)
            .join(', ');
        const rows = await selectRows(`SELECT ${columns} FROM ${definition.table} WHERE ${definition.primaryKey} = ?`, [id]);
        return rows[0] ?? null;
    }
    async create(resource, data) {
        ...
        const [result] = await pool.execute(`INSERT INTO ${definition.table} (${fields.join(', ')}) VALUES (${placeholders})`, fields.map((field) => normalizeDateOnly(data[field]) ?? null));
        const id = Number(result.insertId);
        const record = await this.getById(resource, id);
        if (!record)
            throw new Error('Created web record could not be loaded');
        return record;
    }
    async update(resource, id, data) {
        ...
        const fields = Object.keys(data).filter((key) => definition.columns.includes(key));
        if (fields.length) {
            await pool.execute(`UPDATE ${definition.table} SET ${fields.map((field) => `${field} = ?`).join(', ')} WHERE ${definition.primaryKey} = ?`, [...fields.map((field) => normalizeDateOnly(data[field]) ?? null), id]);
        }
        return this.getById(resource, id);
    }
    async delete(resource, id) {
        const definition = this.definition(resource);
        const [result] = await pool.execute(`DELETE FROM ${definition.table} WHERE ${definition.primaryKey} = ?`, [id]);
        return result.affectedRows > 0;
    }
}
//# sourceMappingURL=WebAdminRepository.js.map