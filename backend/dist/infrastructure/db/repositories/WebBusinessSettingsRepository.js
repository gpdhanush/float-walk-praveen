...
export class WebBusinessSettingsRepository {
    async getStatus() {
        const rows = await selectRows('SELECT closed, reason, updated_at FROM web_store_status WHERE id = 1');
        if (!rows.length) {
    ...
    async getHours() {
        const rows = await selectRows('SELECT id, day, is_closed, TIME_FORMAT(open_time, \'%H:%i\') AS open_time, TIME_FORMAT(close_time, \'%H:%i\') AS close_time, sort_order FROM web_business_hours ORDER BY sort_order');
        return rows.map((row) => ({ ...row, id: Number(row.id), is_closed: asBoolean(row.is_closed), sort_order: Number(row.sort_order), day: String(row.day), open_time: row.open_time ? String(row.open_time) : null, close_time: row.close_time ? String(row.close_time) : null }));
    }
...
//# sourceMappingURL=WebBusinessSettingsRepository.js.map