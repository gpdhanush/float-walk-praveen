...
export class GoogleBusinessService {
    async syncHours(hours) {
        ...
        if (!configured)
            return { configured: false, synced: false, message: 'Google Business Profile is not configured; hours were saved locally.' };
        ...
        if (!accessResponse.ok || !accessData.access_token)
            throw new Error(accessData.error_description || 'Could not obtain Google access token');
        const periods = hours.filter((hour) => !hour.is_closed && hour.open_time && hour.close_time).map((hour) => ({ openDay: hour.day, openTime: hour.open_time, closeDay: hour.day, closeTime: hour.close_time }));
...
//# sourceMappingURL=GoogleBusinessService.js.map