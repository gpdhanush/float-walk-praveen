...
export class InvoiceRepository implements IInvoiceRepository {
  async create(
    data: Omit<Invoice, "createdAt" | "updatedAt" | "deletedAt">,
  ): Promise<Invoice> {
  ...
  async update(id: string, data: Partial<Invoice>): Promise<Invoice | null> {
    ...
    if (fields.length === 0) {
      console.log(
        "[InvoiceRepository] No fields to update, returning current invoice",
      );
    ...
    console.log(
      "[InvoiceRepository] Update completed, fetching updated invoice",
    );
    return this.findById(id);
  }
  ...
  async addPayment(
    data: Omit<Payment, "createdAt" | "updatedAt" | "deletedAt">,
  ): Promise<Payment> {
    const id = data.id ?? randomUUID();
    await pool.execute(
