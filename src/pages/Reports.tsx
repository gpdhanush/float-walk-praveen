...
import { Receipt, TrendingDown, TrendingUp, WalletCards } from 'lucide-react';
  ...
  return (
    <div className="font-sans space-y-6 pb-8">
      ...
      <Card className="rounded-[10px] border-border/80 shadow-sm">
        <CardContent className="flex flex-col gap-5 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
          <div>
            <CardTitle className="text-base">Filter reports</CardTitle>
            <CardDescription className="mt-1">Choose the dates used in the totals and tables below.</CardDescription>
          </div>
          <div className="flex w-full flex-col gap-4 sm:w-auto sm:flex-row sm:items-end">
            <div className="w-full space-y-2 sm:w-[220px]"><Label>From</Label><DatePicker value={from} onChange={setFrom} /></div>
            <div className="w-full space-y-2 sm:w-[220px]"><Label>To</Label><DatePicker value={to} onChange={setTo} /></div>
          </div>
        </CardContent>
      </Card>
      ...
      <Tabs defaultValue="sales" className="space-y-4">
        ...
        <TabsContent value="sales" className="mt-0"><Card className="rounded-[10px] border-border/80 shadow-sm"><CardContent className="p-5 sm:px-6"><DataTable data={filteredInvoices} columns={salesColumns} searchKeys={['invoiceNumber', 'customerName', 'status']} exportFileName="sales-report" /></CardContent></Card></TabsContent>
        <TabsContent value="expenses" className="mt-0"><Card className="rounded-[10px] border-border/80 shadow-sm"><CardContent className="p-5 sm:px-6"><DataTable data={filteredExpenses} columns={expenseColumns} searchKeys={['category', 'description', 'date']} exportFileName="expenses-report" /></CardContent></Card></TabsContent>
      </Tabs>
    </div>
  );
}
