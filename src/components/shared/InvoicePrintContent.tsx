...
export function InvoicePrintContent({ invoice }: { invoice: Invoice }) {
  ...
  const subtotal = Number(inv.subtotal ?? 0) || (inv.items || []).reduce((s, i) => s + (i.total || 0), 0);
  const gstAmount = Number(inv.gstAmount ?? 0) || subtotal * ((inv.gstPercent || 0) / 100);
  ...
  return (
    <>
      ...
      `}</style>
      <div id="invoice-print" className="bg-white p-10 w-full mx-auto" style={{ maxWidth: '210mm', minHeight: '297mm' }}>
          ...
          <div className="flex justify-between items-start">
            ...
            <div className="text-right">
              ...
              <div className="space-y-1">
                ...
                <div className={`text-[10px] px-3 py-1 rounded-full inline-block font-bold uppercase mt-2 ${
                  inv.status === 'paid' ? 'bg-green-100 text-green-700' :
                  inv.status === 'partial' ? 'bg-yellow-100 text-yellow-700' :
                  inv.status === 'hold' ? 'bg-gray-100 text-gray-600' :
        ...
        <table className="w-full text-xs mb-6 border-collapse border border-gray-400">
          <thead>
            <tr className="bg-primary text-primary-foreground">
              <th className="text-left p-2 border border-gray-400">DESCRIPTION</th>
              <th className="text-center p-2 border border-gray-400 w-16">QTY</th>
              <th className="text-right p-2 border border-gray-400 w-24">RATE</th>
              <th className="text-right p-2 border border-gray-400 w-28">AMOUNT</th>
            </tr>
          </thead>
          <tbody>
            {(inv.items || []).map((item, i) => (
              <tr key={i}>
                <td className="p-2 border border-gray-400">{item.productName}</td>
                <td className="p-2 border border-gray-400 text-center font-medium">{item.quantity}</td>
                <td className="p-2 border border-gray-400 text-right">₹{(Number(item.price) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                <td className="p-2 border border-gray-400 text-right font-medium">₹{(Number(item.total) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="ml-auto w-80 border border-gray-400">
          <div className="divide-y divide-gray-400 text-xs">
            <div className="flex justify-between items-center py-1.5 px-2">
              ...
              <span className="font-medium">₹{(Number(subtotal) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            {isGstBill && (
              <div className="flex justify-between items-center py-1.5 px-2">
                <span className="font-semibold">GST ({inv.gstPercent || 0}%)</span>
                <span className="font-medium">₹{(Number(gstAmount) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
              </div>
            )}
            {roundOff !== 0 && (
              <div className="flex justify-between items-center py-1.5 px-2">
                <span className="font-semibold">Round Off</span>
                <span className="font-medium">{roundOff > 0 ? '+' : ''}{(Number(roundOff) || 0).toFixed(2)}</span>
              </div>
            )}
            <div className="flex justify-between items-center py-1.5 px-2 font-medium border-t border-gray-400">
              <span>Grand Total</span>
              <span>₹{(Number(grandTotal) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="flex justify-between items-center py-1.5 px-2 text-red-600">
              <span className="font-semibold">Advance Paid</span>
              <span className="font-medium">-₹{(Number(advancePaid) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
            <div className="flex justify-between items-center py-2 px-2 bg-primary text-primary-foreground font-bold">
              <span>Amount Due</span>
              <span>₹{(Number(amountDue) || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-12">
          ...
          <div className="text-center mt-8 pt-4 border-t border-gray-300" >
            <p className="text-xs font-semibold text-primary">{t('thank_you', settings.language)}</p>
            <p className="text-[10px] text-muted-foreground mt-1">{t('computer_generated', settings.language)}</p>
          </div>
        </div>
      </div>
    </>
